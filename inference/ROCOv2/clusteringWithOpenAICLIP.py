...
# Transform image embeddings to batches
def get_image_embeddings(valid_df):
    image_paths = valid_df["image"].tolist()
    image_embeddings = []

    for i in tqdm(range(0, len(image_paths), 32)):
        batch_paths = image_paths[i:i + 32]
        images = [Image.open(f"./data/medical/test/{path}.jpg").convert("RGB") for path in batch_paths]
...
embeddings = get_image_embeddings(valid_df)
...
fig, ax = plt.subplots(num_rows, 3, figsize=(15, 6 * num_rows))
